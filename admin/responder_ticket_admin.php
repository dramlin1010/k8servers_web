...
if (
    ...
    if (empty($contenido_respuesta)) {
        ...
        try {
            $admin_user_id_for_message = isset($_SESSION['AdminID'])
                ? (int) $_SESSION['AdminID']
                : null;

            if ($admin_user_id_for_message !== null) {
                $sql_insert_respuesta =
                    'INSERT INTO Mensaje_Ticket (TicketID, UsuarioID, EsAdmin, Contenido, FechaEnvio)
                                         VALUES (?, ?, TRUE, ?, NOW())';
                $stmt_insert_respuesta = $conn->prepare($sql_insert_respuesta);
                if (!$stmt_insert_respuesta) {
                    throw new Exception(
                        'Error al preparar la inserción de respuesta (con AdminID): ' .
                            $conn->error
                    );
                }
                $stmt_insert_respuesta->bind_param(
                    'iis',
                    $ticketID_ver,
                    $admin_user_id_for_message,
                    $contenido_respuesta
                );
            } else {
                $sql_insert_respuesta =
                ...
                if (!$stmt_insert_respuesta) {
                    throw new Exception(
                        'Error al preparar la inserción de respuesta (AdminID nulo): ' .
                            $conn->error
                    );
                }
                $stmt_insert_respuesta->bind_param(
            ...
            if (!$stmt_update_ticket->execute()) {
                throw new Exception(
                    'Error al actualizar el ticket: ' .
                        $stmt_update_ticket->error
                );
            }
            $stmt_update_ticket->close();

            $conn->commit();
            $success_message_ticket_admin =
                'Respuesta enviada y ticket actualizado.';
        } catch (Exception $e) {
            $conn->rollback();
...
if ($stmt_ticket) {
    ...
    if ($result_ticket->num_rows === 1) {
        ...
        if ($stmt_mensajes) {
            ...
            $error_message_ticket_admin =
                'Error al cargar mensajes del ticket: ' . $conn->error;
        }
    } else {
        $_SESSION['error_message_ticket_list_admin'] =
            'Ticket no encontrado o datos de cliente inaccesibles.';
        $stmt_ticket->close();
        if ($conn) {
            $conn->close();
        }
    ...
    $error_message_ticket_admin =
        'Error al preparar la consulta del ticket: ' . $conn->error;
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <title>Responder Ticket #<?php echo htmlspecialchars(
        (string) ($ticketID_ver ?? '')
    ); ?> - Admin</title>
        ...
        <main class="panel-main-content animated-section">
            <header class="panel-main-header">
                <div class="container-fluid">
                    ...
                    <p>Ticket #<?php echo htmlspecialchars(
                        (string) ($ticket_detalle['TicketID'] ?? 'N/A')
                    ); ?> - Asunto: "<?php echo htmlspecialchars(
    (string) ($ticket_detalle['Asunto'] ?? 'Sin asunto')
); ?>"</p>
                    <?php endif; ?>
                </div>
            </header>
                ...
                <div class="container-fluid">
                    ...
                    if (!empty($success_message_ticket_admin)) {
                        echo '<div class="alert alert-success">' .
                            htmlspecialchars($success_message_ticket_admin) .
                            '</div>';
                    }
                    if (!empty($error_message_ticket_admin)) {
                        echo '<div class="alert alert-danger">' .
                            htmlspecialchars($error_message_ticket_admin) .
                            '</div>';
                    }
                    ?>
                    <?php if ($ticket_detalle):
                        $ticketID_display = htmlspecialchars(
                            (string) ($ticket_detalle['TicketID'] ?? 'N/A')
                        );
                        $clienteID_raw = $ticket_detalle['ClienteID'] ?? null;
                        $clienteNombre_display = htmlspecialchars(
                            trim(
                                ($ticket_detalle['ClienteNombre'] ?? '') .
                                    ' ' .
                                    ($ticket_detalle['ClienteApellidos'] ??
                                        'Cliente Desconocido')
                            )
                        );
                        $clienteEmail_display = htmlspecialchars(
                            (string) ($ticket_detalle['ClienteEmail'] ?? 'N/A')
                        );
                        $clienteLink = $clienteID_raw
                            ? 'ver_usuario_admin.php?id=' .
                                htmlspecialchars((string) $clienteID_raw)
                            : '#';

                        $asunto_display = htmlspecialchars(
                            (string) ($ticket_detalle['Asunto'] ?? 'N/A')
                        );
                        $sitioID_raw = $ticket_detalle['SitioID'] ?? null;
                        $sitioDominio_display = htmlspecialchars(
                            (string) ($ticket_detalle['SitioDominio'] ?? 'N/A')
                        );
                        $sitioLink =
                            $sitioID_raw && $ticket_detalle['SitioDominio']
                                ? 'ver_sitio_admin.php?id=' .
                                    htmlspecialchars((string) $sitioID_raw)
                                : null;

                        $prioridad_display = ucfirst(
                            htmlspecialchars(
                                (string) ($ticket_detalle['Prioridad'] ?? 'N/A')
                            )
                        );
                        $prioridad_class = strtolower(
                            htmlspecialchars(
                                (string) ($ticket_detalle['Prioridad'] ?? '')
                            )
                        );

                        $estado_display = ucfirst(
                            str_replace(
                                '_',
                                ' ',
                                htmlspecialchars(
                                    (string) ($ticket_detalle['Estado'] ??
                                        'N/A')
                                )
                            )
                        );
                        $estado_class = strtolower(
                            str_replace(
                                '_',
                                '-',
                                htmlspecialchars(
                        ...
                        $estado_actual_form = htmlspecialchars(
                            (string) ($ticket_detalle['Estado'] ?? '')
                        );

                        $fechaCreacion_display = isset(
                            $ticket_detalle['FechaCreacion']
                        )
                            ? date(
                                'd/m/Y H:i',
                                strtotime($ticket_detalle['FechaCreacion'])
                            )
                            : 'N/A';
                        $ultimaActualizacion_display = isset(
                            $ticket_detalle['UltimaActualizacion']
                        )
                            ? date(
                                'd/m/Y H:i',
                                strtotime(
                            ...
                            <?php else: ?>
                                <?php foreach ($mensajes_ticket as $mensaje):
                                    ...
                                    if ($mensaje['EsAdmin']) {
                                        ...
                                        isset($mensaje['MsgClienteNombre']) ||
                                        isset($mensaje['MsgClienteApellidos'])
                                    ) {
                                        $nombre_emisor = htmlspecialchars(
                                            trim(
                                                ($mensaje['MsgClienteNombre'] ??
                                                    '') .
                                                    ' ' .
                                                    ($mensaje[
                                                        'MsgClienteApellidos'
                                        ...
                                        $nombre_emisor =
                                            'Cliente (ID: ' .
                                            htmlspecialchars(
                                                (string) $mensaje['UsuarioID']
                                            ) .
                                            ')';
                                    }
                                    ?>
                                <div class="message-item <?php echo $mensaje[
                                    'EsAdmin'
                                ]
                                    ? 'admin-reply'
                                    : 'client-message'; ?>">
                                    <div class="message-header">
                                        ...
                                        respondió el <?php echo isset(
                                            $mensaje['FechaEnvio']
                                        )
                                            ? date(
                                                'd/m/Y H:i',
                                                strtotime($mensaje['FechaEnvio'])
                                            )
                                            : 'Fecha desconocida'; ?>
                                    </div>
                            ...
                            <form action="responder_ticket_admin.php?id=<?php echo $ticketID_ver; ?>" method="POST" class="styled-form">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="nuevo_estado_ticket" name="nuevo_estado_ticket">
                                        ...
                                        <option value="abierto" <?php if (
                                            $estado_actual_form === 'abierto'
                                        ) {
                                            echo 'disabled';
                                        } ?>>Abierto</option>
                                        <option value="en_progreso" <?php if (
                                            $estado_actual_form ===
                                            'en_progreso'
                                        ) {
                                            echo 'disabled';
                                        } ?>>En Progreso</option>
                                        <option value="esperando_cliente" <?php if (
                                            $estado_actual_form ===
                                            'esperando_cliente'
                                        ) {
                                            echo 'disabled';
                                        } ?>>Esperando Respuesta del Cliente</option>
                                        <option value="resuelto" <?php if (
                                            $estado_actual_form === 'resuelto'
                                        ) {
                                            echo 'disabled';
                                        } ?>>Resuelto</option>
                                        <option value="cerrado" <?php if (
                                            $estado_actual_form === 'cerrado'
                                        ) {
                                            echo 'disabled';
                                        } ?>>Cerrado</option>
                                    </select>
